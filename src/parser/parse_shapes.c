...
/*   Updated: 2023/05/19 16:38:05 by mdias-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	parse_sphere(t_scanner *scanner, t_scene *scene)
{
	...
	shape = new_sphere();
	if (!parse_position(scanner, &shape.sphere.origin))
		return (FALSE);
	if (!parse_dimension(scanner, &shape.sphere.radius))
		return (FALSE);
	...
	shape.transform = multiply_mx_mx(
			translation(
				shape.sphere.origin.x,
				shape.sphere.origin.y,
				shape.sphere.origin.z),
			scaling(
				shape.sphere.radius,
				shape.sphere.radius,
				shape.sphere.radius
				));
	add_to_world(&shape, &scene->world);
	return (TRUE);
}
...
t_bool	parse_plane(t_scanner *scanner, t_scene *scene)
{
	...
	shape = new_plane();
	if (!parse_position(scanner, &shape.plane.origin))
		return (FALSE);
